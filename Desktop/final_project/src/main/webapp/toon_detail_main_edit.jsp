...
while (rs.next())
{%>
<form action="toon_detail_main_save.jsp" enctype ="multipart/form-data" method="post">
<span class="buttons">
<span> <button type="submit" id="toon_edit">등록하기</button></span>
<span> <input type="button" value ="취소" id="main_edit" onclick ="cancel()">  </span>
</span>
<table frame= void>
	<tr class="mains">  
		<td>
		<span> <input type="text" id="idx" name="idx" value ="<%=rs.getInt("idx")%>"></span>
		<img src="./upload/<%=rs.getString("savedFileName")%>" style="width:200px; height:200px; text-align:center; border-radius:7px;">
		</td>
		<td>
		<div id="titles" style="font-size:30px; font-weight:600; padding:5px; font-align:left;" ><input type="text" name="title" style="" id="title"value ="<%=rs.getString("title")%>"></div>
		 <div id="sub"><input type="text" name="writer" id="writer"value ="<%=rs.getString("writer")%>"></div>
		  <div id="sub"><input type="text" name="genre" id="genre"value ="<%=rs.getString("genre")%>"></div>
		   <div id="sub"><input type="text" style="width:300px; height:100px;" name="content" id="content"value ="<%=rs.getString("content")%>"></div>
		   <div id= "sub"> <input type="file" accept="image/jpg" id="fileName" name="fileName"></div>
		</td>
	</tr>
	</form>
	<tr class ="subs">
